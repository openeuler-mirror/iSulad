 ...
 * Description: oci_rootfs_mount llt
 * Author: wangfengtu
 * Create: 2019-08-26
 */
...
{
    rootfs_mount_request *req = NULL;
    rootfs_mount_response *resp = NULL;
    int i = 0;

    // Test parameter NULL
    ASSERT_NE(mount_rootfs(NULL, NULL), 0);
    ASSERT_NE(mount_rootfs(NULL, &resp), 0);
    free_rootfs_mount_response(resp);
    resp = NULL;

    // Test content of parameter NULL
    req = (rootfs_mount_request*)util_common_calloc_s(sizeof(rootfs_mount_request));
    ASSERT_TRUE(req != NULL);
    ASSERT_NE(mount_rootfs(req, &resp), 0);
    free_rootfs_mount_request(req);
    req = NULL;
    free_rootfs_mount_response(resp);
    resp = NULL;
    ...
    ASSERT_EQ(mount_rootfs(req, &resp), 0);
...
TEST(oci_rootfs_mount_llt, test_free_rootfs_mount_request)
{
    rootfs_mount_request *req = NULL;

    // Test free NULL
    free_rootfs_mount_request(NULL);
...
TEST(oci_rootfs_mount_llt, test_free_rootfs_mount_response)
{
    rootfs_mount_response *resp = NULL;

    // Test free NULL
    free_rootfs_mount_response(NULL);

    // Test free errmsg NULL
    resp = (rootfs_mount_response*)util_common_calloc_s(sizeof(rootfs_mount_response));
    ASSERT_TRUE(resp != NULL);
