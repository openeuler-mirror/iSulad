                               ...
                               char **stderr_msg)
{
    ...
    if (g_exec_cmd_count == 3) {
        *stderr_msg = util_strdup_s("container not known");
        return false;
    }
...
TEST(oci_rootfs_remove_llt, test_remove_rootfs)
{
    rootfs_remove_request *req = NULL;
    rootfs_remove_response *resp = NULL;
    int i = 0;

    // Test parameter NULL
    ASSERT_NE(remove_rootfs(NULL, NULL), 0);
    ASSERT_NE(remove_rootfs(NULL, &resp), 0);
    free_rootfs_remove_response(resp);
    resp = NULL;

    // Test content of parameter NULL
    req = (rootfs_remove_request*)util_common_calloc_s(sizeof(rootfs_remove_request));
    ASSERT_TRUE(req != NULL);
    ASSERT_NE(remove_rootfs(req, &resp), 0);
    free_rootfs_remove_request(req);
    req = NULL;
    free_rootfs_remove_response(resp);
    resp = NULL;
    ...
    ASSERT_EQ(remove_rootfs(req, &resp), 0);
    ...
    // 1. Test util_exec_cmd failed with stderr_buffer == NULL
    // 3. Test util_exec_cmd failed with stderr_buffer contains "container not known"
    for (i = 0; i < 3; i++) {
        req = (rootfs_remove_request*)util_common_calloc_s(sizeof(rootfs_remove_request));
        ASSERT_TRUE(req != NULL);
        req->name_id = util_strdup_s("image_name");
        ...
        MOCK_SET_DEFAULT_ISULAD_KIT_OPTS
        if (i == 2) {
            // If stderr_buffer contains "container not known",
            // remove_rootfs must return success.
            ASSERT_EQ(remove_rootfs(req, &resp), 0);
        } else {
            ASSERT_NE(remove_rootfs(req, &resp), 0);
        }
...
TEST(oci_rootfs_remove_llt, test_free_rootfs_remove_request)
{
    rootfs_remove_request *req = NULL;

    // Test free NULL
    free_rootfs_remove_request(NULL);
...
TEST(oci_rootfs_remove_llt, test_free_rootfs_remove_response)
{
    rootfs_remove_response *resp = NULL;

    // Test free NULL
    free_rootfs_remove_response(NULL);

    // Test free errmsg NULL
    resp = (rootfs_remove_response*)util_common_calloc_s(sizeof(rootfs_remove_response));
    ASSERT_TRUE(resp != NULL);
