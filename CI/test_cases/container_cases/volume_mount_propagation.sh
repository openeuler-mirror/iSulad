...
function test_volume_mount_default()
{
  ...
  CONT=$(isula run --mount type=bind,src=/tmp/src,dst=/tmp/dst -dit --privileged "${image}" /bin/sh)
...
function test_volume_mount_private() {
    ...
    #run with slave property volume in container
    CONT=$(isula run --mount type=bind,src=/tmp/src,dst=/tmp/dst,bind-propagation=private -dit --privileged "${image}" /bin/sh)
    [[ $? -ne 0 ]] && msg_err "${FUNCNAME[0]}:${LINENO} - run container by image $image failed" && ((ret++))
...
function test_volume_mount_rprivate() {
    ...
    #run with slave property volume in container
    CONT=$(isula run --mount type=bind,src=/tmp/src,dst=/tmp/dst,bind-propagation=rprivate -dit --privileged "${image}" /bin/sh)
    [[ $? -ne 0 ]] && msg_err "${FUNCNAME[0]}:${LINENO} - run container by image $image failed" && ((ret++))
...
function test_volume_mount_rshared() {
    ...
    #run with slave property volume in container
    CONT=$(isula run --mount type=bind,src=/tmp/src,dst=/tmp/dst,bind-propagation=rshared -dit --privileged "${image}" /bin/sh)
    [[ $? -ne 0 ]] && msg_err "${FUNCNAME[0]}:${LINENO} -run container by image $image failed" && ((ret++))
...
function test_volume_mount_rslave() {
    ...
    #run with slave property volume in container
    CONT=$(isula run --mount type=bind,src=/tmp/src,dst=/tmp/dst,bind-propagation=rslave -dit --privileged "${image}" /bin/sh)
    [[ $? -ne 0 ]] && msg_err "${FUNCNAME[0]}:${LINENO} - run container by image $image failed" && ((ret++))
...
function test_volume_mount_shared() {
    ...
    CONT=$(isula run --mount type=bind,src=/tmp/src,dst=/tmp/dst,bind-propagation=shared -dit --privileged "$image" /bin/sh)
    [[ $? -ne 0 ]] && msg_err "${FUNCNAME[0]}:${LINENO} - run container by image $image failed" && ((ret++))
...
function test_volume_mount_slave() {
    ...
    #run with slave property volume in container
    CONT=$(isula run --mount type=bind,src=/tmp/src,dst=/tmp/dst,bind-propagation=slave -dit --privileged "${image}" /bin/sh)
    [[ $? -ne 0 ]] && msg_err "${FUNCNAME[0]}:${LINENO} - run container by image $image failed" && ((ret++))
