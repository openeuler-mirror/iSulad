    ...
    */
    ...
    if (version != NULL && !util_strings_contains_word(version, "registry/2.0")) {
        WARN("Docker-Distribution-Api-Version does not contain registry/2.0, it's value is %s."
             "Registry may can not support registry API V2",
             version);
    }
...
static int search_parse_challenges(search_descriptor *desc, char *schema, char *params)
{
    challenge c = { 0 };
    char **kv_strs = NULL;
    char **kv = NULL;
...
static int search_parse_auth(search_descriptor *desc, char *auth)
{
    char *origin_tmp_auth = NULL;
    char *trimmed_auth = NULL;
    int ret = 0;
    ...
    ret = search_parse_challenges(desc, schema, params);
    if (ret != 0) {
        ERROR("Parse challenges failed, schema: %s, params: %s", schema, params);
        ret = -1;
        goto out;
...
static int search_parse_auths(search_descriptor *desc, struct parsed_http_message *m)
{
    int i = 0;
    int ret = 0;
    ...
    for (i = 0; i < m->num_headers; i++) {
        if (!strcasecmp(m->headers[i][0], "Www-Authenticate")) {
            ret = search_parse_auth(desc, (char *)m->headers[i][1]);
            if (ret != 0) {
                WARN("parse auth %s failed", (char *)m->headers[i][1]);
            }
        }
...
static int search_parse_ping_header(search_descriptor *desc, char *http_head)
{
    struct parsed_http_message *message = NULL;
    char *version = NULL;
    int ret = 0;
    ...
    */
    ...
    ret = search_parse_auths(desc, message);
    if (ret != 0) {
        ERROR("Parse WWW-Authenticate header failed");
        goto out;
    }
...
int search_pingv2(search_descriptor *desc, char *protocol)
{
    int ret = 0;
    int sret = 0;
    char *output = NULL;
    ...
    // https://registry.isula.org/v2/
    INFO("sending ping url: %s", url);
    ret = http_request_buf_for_search(desc, url, (const char **)headers, &output, HEAD_BODY);
    if (ret != 0) {
        ERROR("http request failed");
        goto out;
    }
    ...
    ret = search_parse_ping_header(desc, output);
    if (ret != 0) {
        ERROR("parse ping header failed, response: %s", output);
        goto out;
    }
...
static int search_ping(search_descriptor *desc)
{
    int ret = 0;
...
int fetch_tags(search_descriptor *desc, char **output_buffer)
{
    ...
    sret = snprintf(path, sizeof(path), "/v2/%s/tags/list", desc->image_name);

    if (sret < 0 || (size_t)sret >= sizeof(path)) {
        ERROR("Failed to sprintf path for tags");
        ret = -1;
        goto out;
    }
