...
static int http_request_get_token_for_search(search_descriptor *desc, challenge *c, char **output);
...
static int search_setup_ssl_config(search_descriptor *desc, struct http_get_options *options, const char *url)
{
    int ret = 0;
    char *host = NULL;
...
static int search_setup_auth_basic(search_descriptor *desc, char ***custom_headers)
{
    int ret = 0;
    char *auth_header = NULL;
...
static int search_get_bearer_token(search_descriptor *desc, challenge *c)
{
    int ret = 0;
    char *output = NULL;
    time_t now = time(NULL);
    ...
    ret = http_request_get_token_for_search(desc, c, &output);
    if (ret != 0 || output == NULL) {
        ERROR("http request get token failed, result is %d", ret);
        ret = -1;
        goto out;
...
static int search_setup_auth_challenges(search_descriptor *desc, char ***custom_headers)
{
    int ret = 0;
    int i = 0;
    char *auth_header = NULL;
    ...
    for (i = 0; i < CHALLENGE_MAX; i++) {
        ...
        if (!strcasecmp(desc->challenges[i].schema, "Basic")) {
            ...
            (void)pthread_mutex_lock(&desc->challenges_mutex);
            ret = search_get_bearer_token(desc, &desc->challenges[i]);
            if (ret != 0) {
                (void)pthread_mutex_unlock(&desc->challenges_mutex);
                ERROR("get bearer token failed");
                isulad_try_set_error_message("authentication failed");
                                       ...
                                       const char **custom_headers)
{
    ...
    // Add https related options
    ret = search_setup_ssl_config(desc, options, url);
    if (ret != 0) {
        ERROR("Failed setup ssl config");
        isulad_try_set_error_message("setup ssl config failed");
        ret = -1;
    ...
    ret = search_setup_auth_challenges(desc, &options->custom_headers);
    if (ret != 0) {
        ERROR("setup auth challenges failed");
        isulad_try_set_error_message("setup auth challenges failed");
        ret = -1;
...
static int search_setup_get_token_options(search_descriptor *desc, struct http_get_options *options, const char *url)
{
    ...
    ret = search_setup_auth_basic(desc, &options->custom_headers);
    if (ret != 0) {
        ERROR("dup headers failed");
        ret = -1;
        goto out;
...
static int http_request_get_token_for_search(search_descriptor *desc, challenge *c, char **output)
{
    ...
    ret = search_setup_get_token_options(desc, options, c->realm);
    if (ret != 0) {
        ERROR("Failed setup common options");
        ret = -1;
        goto out;
                                ...
                                resp_data_type type)
{
    ...
    ret = search_setup_common_options(desc, options, url, custom_headers);
    if (ret != 0) {
        ERROR("Failed setup common options");
        ret = -1;
        goto out;
