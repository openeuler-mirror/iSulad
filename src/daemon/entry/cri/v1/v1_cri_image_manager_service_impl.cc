...
static void conv_image_to_grpc(const imagetool_image_summary *element, std::unique_ptr<runtime::v1::Image> &image)
{
    if (element == nullptr) {
        return;
    }
    ...
    if (element->uid != nullptr) {
        runtime::v1::Int64Value *uid_value = new (std::nothrow) runtime::v1::Int64Value;
...
auto ImageManagerServiceImpl::pull_request_from_grpc(const runtime::v1::ImageSpec *image,
                                                     const runtime::v1::AuthConfig *auth, im_pull_request **request,
                                                     Errors &error) -> int
{
...
auto ImageManagerServiceImpl::list_request_from_grpc(const runtime::v1::ImageFilter *filter,
                                                  ...
                                                  Errors &error)
{
    ...
    for (size_t i = 0; i < list_images->images_len; i++) {
        std::unique_ptr<runtime::v1::Image> image(new (std::nothrow) runtime::v1::Image);
        if (image == nullptr) {
            error.SetError("Out of memory");
            return;
        }
...
auto ImageManagerServiceImpl::status_request_from_grpc(const runtime::v1::ImageSpec *image,
                                                                               ...
                                                                               Errors &error)
{
    ...
    std::unique_ptr<runtime::v1::Image> out { nullptr };
...
auto ImageManagerServiceImpl::PullImage(const runtime::v1::ImageSpec &image,
                                        const runtime::v1::AuthConfig &auth, Errors &error) -> std::string
{
    std::string out_str;
...
auto ImageManagerServiceImpl::remove_request_from_grpc(const runtime::v1::ImageSpec *image,
...
void ImageManagerServiceImpl::RemoveImage(const runtime::v1::ImageSpec &image, Errors &error)
{
    std::string out_str;
                                              ...
                                              Errors & /*error*/)
{
    ...
    for (size_t i {}; i < got_fs_info->image_filesystems_len; i++) {
        using FilesystemUsagePtr = std::unique_ptr<runtime::v1::FilesystemUsage>;
        FilesystemUsagePtr fs_info(new (std::nothrow) runtime::v1::FilesystemUsage);
        if (fs_info == nullptr) {
            ERROR("Out of memory");
            return;
        }
        ...
        if (element->fs_id != nullptr && element->fs_id->mountpoint != nullptr) {
            runtime::v1::FilesystemIdentifier *fs_id = new (std::nothrow) runtime::v1::FilesystemIdentifier;
            if (fs_id == nullptr) {
                ERROR("Out of memory");
                return;
            }
        ...
        if (element->used_bytes != nullptr) {
            runtime::v1::UInt64Value *used_bytes = new (std::nothrow) runtime::v1::UInt64Value;
        ...
        if (element->inodes_used != nullptr) {
            runtime::v1::UInt64Value *inodes_used = new (std::nothrow) runtime::v1::UInt64Value;
