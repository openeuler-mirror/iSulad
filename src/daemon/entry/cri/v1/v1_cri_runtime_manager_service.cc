...
auto RuntimeManagerService::Status(Errors &error) -> std::unique_ptr<runtime::v1::RuntimeStatus>
{
    std::unique_ptr<runtime::v1::RuntimeStatus> status(new (std::nothrow) runtime::v1::RuntimeStatus);
    if (status == nullptr) {
        error.SetError("Out of memory");
        return nullptr;
    }

    runtime::v1::RuntimeCondition *runtimeReady = status->add_conditions();
    runtimeReady->set_type(CRIHelpers::Constants::RUNTIME_READY);
    runtimeReady->set_status(true);
    runtime::v1::RuntimeCondition *networkReady = status->add_conditions();
