    ...
    const runtime::v1alpha2::ContainerConfig &containerConfig, host_config *hostconfig, Errors &error) -> int
{
    ...
    CRIHelpersV1Alpha::AddSecurityOptsToHostConfig(securityOpts, hostconfig, error);
    if (error.NotEmpty()) {
                                                         ...
                                                         host_config *hostconfig, const std::string &realPodSandboxID, Errors &error) -> int
{
    ...
    selinuxLabelOpts = CRIHelpersV1Alpha::GetPodSELinuxLabelOpts(tmp_str, error);
    if (error.NotEmpty()) {
    ...
    const std::string &realPodSandboxID, Errors &error) -> host_config *
{
    ...
    CRIHelpersV1Alpha::GenerateMountBindings(containerConfig.mounts(), hostconfig, error);
    if (error.NotEmpty()) {
        goto cleanup;
    }
                                                  ...
                                                  container_config *cConfig, Errors &error)
{
    ...
    if (config.envs_size() > 0) {
        ...
        auto envVect = CRIHelpersV1Alpha::GenerateEnvList(config.envs());
                                                        ...
                                                        const std::string &podSandboxRuntime, Errors &error)
{
    ...
    CRIHelpersV1Alpha::UpdateCreateConfig(custom_config, hostconfig, containerConfig, realPodSandboxID, error);
    if (error.NotEmpty()) {
        goto cleanup;
    }
    ...
    if (filter != nullptr) {
        ...
        if (filter->has_state()) {
            if (CRIHelpers::FiltersAdd((*request)->filters, "status",
                                       CRIHelpersV1Alpha::ToIsuladContainerStatus(filter->state())) != 0) {
                error.SetError("Failed to add filter");
                return;
            }
        }
