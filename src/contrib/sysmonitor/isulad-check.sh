...
#backtrace定位日志存放目录
log_dir="/var/lib/lcrd/isulad-monitor"
#backtrace定位日志占用总大小，单位为KB
default_size="10240"
#backtrace定位日志总个数
default_num="4"
#健康检查文件
...
#检查backtrace日志文件个数是否大于5个
checkFileNum(){
        num=`ls $log_dir | wc -l`
        rm_num=$(($num-$default_num))
	k=2

        if [ $rm_num -gt 0 ]; then
		for ((i=1;i<=$rm_num;i++))
                do
			rm -f $log_dir/$(ls -rt -l $log_dir | head -$k | tail -1 | awk '{print $9}')
			if [ $? -ne 0 ];then
				k=$((k+1))
			fi
                done
...
#检查backtrace日志文件总大小是否大于10M
checkFileSize(){
	num=`ls $log_dir | wc -l`
	k=2

	#文件夹本身占用4KB
        size=$((`du -sk $log_dir| awk '{print $1}'`-4))
        while [ $size -ge $default_size ]
        do
		if [ $num -eq 0 ];then
			break;
		fi
...
#维护/var/lib/lcrd/isulad-monitor目录下的log文件
#限制stack log的存储空间占用
manage_monitor_log(){
	checkFileNum
	if [ $? -ne 0 ];then
        	echo "Failed to checkfilenum"
        	return 1;
	fi

	checkFileSize
	if [ $? -ne 0 ];then
        	echo "Failed to checkfilesize"
        	return 1;
	fi
}
...
case "$file_status" in
	...
	success)
		...
		exit 0
		;;	
	##上次的检查已经超时，代表失败，返回1，需要重新由sysmonitor restart服务
	failed)
		clean_healthcheck
		manage_monitor_log
		sleep 5
